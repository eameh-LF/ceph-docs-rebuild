...
4. Man pages that duplicate reference pages. rbd, ceph, radosgw-admin, ceph-volume, cephfs-top and cephadm each have both a man page and a prose page covering the same commands. Recommendation: keep the man pages, and make each one the single source for its command reference. The man source stays plain RST so it renders to roff; prose pages include a command's entry from it at build time (see PR 71609) instead of restating or merely linking; retire the rbd/man/index and rados/man/index hubs; add the six man pages missing from man_index.rst.
...
12. iSCSI gateway. 13 pages plus cephadm/services/iscsi for a gateway in maintenance mode. Recommendation: one retirement page. iSCSI is unmaintained and not recommended; NVMe-oF is the supported path; prior-release docs serve anyone still running it; a guarded link to the PetaSAN fork (github.com/FredNass/ceph_petasan) for those stuck on it. The 13 pages and cephadm/services/iscsi come out of the tree.
