...
Command reference is single-sourced from the man pages, not turned into stubs. The man page source stays plain so it still renders to roff, and prose pages pull a command's entry from it at build time instead of restating or merely linking to it, with an author comment on both sides. The first live trial is [PR 71609](https://github.com/ceph/ceph/pull/71609), the rbd snapshots page.
...
- Review the door map with Ville before any index changes; the toctree mechanics are his territory.
